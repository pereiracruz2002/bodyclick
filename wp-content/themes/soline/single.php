              ...
              <h1><?=the_title();?></h1>
                 ...
                 <div class="pikachoose" style="width:320px">
                    ...
                    <ul id="pikame" class="jcarousel-skin-pika pika-thumbs">
                     <?php 
                      foreach($array_images as $foto):
                      $explode_thumb = explode("/",$foto);
                      $final_thumb = explode(".",$explode_thumb[7]);
                      $caminho_img_thumb = $_SERVER['DOCUMENT_ROOT']."/".$explode_thumb[3]."/".$explode_thumb[4]."/".$explode_thumb[5]."/".$explode_thumb[6]."/".$final_thumb[0]."-400x400.".$final_thumb[1];

                      if(file_exists($caminho_img_thumb)){
                         $foto_thumb = $explode_thumb[0]."/".$explode_thumb[1]."/".$explode_thumb[2]."/".$explode_thumb[3]."/".$explode_thumb[4]."/".$explode_thumb[5]."/".$explode_thumb[6]."/".$final_thumb[0]."-400x400.".$final_thumb[1];
                      }else{
                        $foto_thumb = $explode_thumb[0]."/".$explode_thumb[1]."/".$explode_thumb[2]."/".$explode_thumb[3]."/".$explode_thumb[4]."/".$explode_thumb[5]."/".$explode_thumb[6]."/".$final_thumb[0].".".$final_thumb[1];
                      }
                     ?>
                      <li><div class="clip"><a href="#"><img src="<?=$foto_thumb?>" alt="<?=$array_images_gd[$i]?>" class="foto"/></a></div></li>
                     <?php $i++;
                     endforeach; ?>
                    </ul>
                 </div>
